...
function colorPalette (input, type, qty, callback) {
  ...
  // PNG, GIF, JPG
  return paletteFromBitmap(input, type, qty, callback)
}

function paletteFromBitmap (filename, type, qty, callback) {
  if (!callback) {
    callback = type
    type = null
  }
  ...
  getPixels(filename, type, function (err, pixels) {
    ...
    const palette = getRgbaPalette(pixels.data, qty).map(function (rgba) {
      return chroma(rgba)
    })
